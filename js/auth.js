...
const authModule = {
  ...
  async register(userData) {
    try {
      ...
      // Автоматически входим после регистрации (если не требуется подтверждение email)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setTimeout(() => {
    authModule.init().then(() => {
      ...
      if (authModule.isAuthenticated() && typeof app !== 'undefined') {
        setTimeout(() => {
          app.showMain();
        }, 500);
      }
